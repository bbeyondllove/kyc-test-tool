...
class LivePortraitPipeline(object):
    ...
    def execute(self, args: ArgumentConfig):
        ...
        if flag_load_from_template:
            ...
            # 保存pkl缓存到可写目录（Docker环境中driving目录为只读）
            import os
            cache_dir = os.environ.get('LIVEPORTRAIT_CACHE_DIR', '/app/cache/liveportrait')
            os.makedirs(cache_dir, exist_ok=True)
            driving_basename = os.path.basename(remove_suffix(args.driving))
            wfp_template = os.path.join(cache_dir, driving_basename + '.pkl')
        ...
        if flag_is_driving_video or (flag_is_source_video and not flag_is_driving_video):
            ...
            # 只在启用 flag_write_concat 时生成 concat 视频
            if args.flag_write_concat:
            ...
            if args.flag_write_concat and wfp_concat:
